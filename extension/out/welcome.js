...
function showWelcome(context) {
    const panel = vscode.window.createWebviewPanel('orkestra.welcome', 'Welcome to Orkestra IDE', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: false });
...
function registerWelcome(context) {
    ...
    const hasSeenWelcome = context.globalState.get('orkestra.welcomeSeen');
    if (!hasSeenWelcome) {
