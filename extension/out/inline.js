    ...
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('orkestra.inlineEdit', async (editor) => {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                const action = await vscode.window.showInformationMessage(`Orkestra finished: ${result.summary || instruction}`, 'Apply', 'Show Diff', 'Dismiss');
                if (action === 'Apply') {
...
class OrkestaChatPanel {
    static show(context, api, initialContext) {
        ...
        const panel = vscode.window.createWebviewPanel('orkestra.chat', 'Orkestra Chat', vscode.ViewColumn.Two, {
            enableScripts: true,
            ...
            switch (msg.type) {
                ...
                case 'startRun': {
                    try {
                        ...
                        OrkestaChatPanel._ws = api.openWebSocket((ev) => { panel.webview.postMessage({ type: 'wsEvent', event: ev }); }, () => { });
                    }
                    catch (e) {
                ...
                case 'cancelRun':
                    try {
                        await api.cancelRun(msg.runId);
                    }
                    catch { }
                    if (OrkestaChatPanel._ws) {
                        try {
                            OrkestaChatPanel._ws.close();
                        }
                        catch { }
...
OrkestaChatPanel._ws = null;
async function handleChatMessage(panel, api, message, codeContext, model) {
...
function getChatHtml(webview, context) {
    const jsUri = webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, 'media', 'chat.js'));
    const csp = [
        `default-src 'none'`,
