...
exports.OrkestaChatPanel = void 0;
...
function registerInlineEdit(context, api) {
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('cesaflow.inlineEdit', async (editor) => {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                const action = await vscode.window.showInformationMessage(`CesaFlow finished: ${result.summary || instruction}`, 'Apply', 'Show Diff', 'Dismiss');
                if (action === 'Apply') {
...
async function showDiff(doc, newContent, sel, context) {
    ...
    vscode.commands.executeCommand('vscode.diff', originalUri, modifiedUri, 'CesaFlow: Before ↔ After');
}
// ── Cmd+L Chat Panel ──────────────────────────────────────────────────────
class OrkestaChatPanel {
    static show(context, api, initialContext) {
        ...
        const panel = vscode.window.createWebviewPanel('cesaflow.chat', 'CesaFlow Chat', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
...
exports.OrkestaChatPanel = OrkestaChatPanel;
async function handleChatMessage(panel, api, message, codeContext) {
...
function registerChatPanel(context, api) {
    context.subscriptions.push(vscode.commands.registerCommand('cesaflow.chat', () => {
        const editor = vscode.window.activeTextEditor;
        let ctx = '';
        if (editor) {
            const sel = editor.selection;
...
<body>
  <div id="header">⚡ CesaFlow Chat</div>
  <div id="messages"></div>
