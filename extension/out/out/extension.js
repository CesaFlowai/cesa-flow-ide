...
function activate(context) {
    const api = new api_1.CesaFlowApi();
    const panel = new panel_1.CesaFlowPanel(context, api);
    // ── Commands ──────────────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand('cesaflow.newRun', async () => {
        const selection = getEditorSelection();
        panel.show();
        panel.triggerNewRun(selection);
    }), vscode.commands.registerCommand('cesaflow.openPanel', () => {
        panel.show();
    }), vscode.commands.registerCommand('cesaflow.sendSelection', async () => {
        const selection = getEditorSelection();
        if (!selection) {
        ...
        panel.show();
        panel.triggerNewRun(selection);
    }), vscode.commands.registerCommand('cesaflow.configure', async () => {
        ...
        if (key) {
            await vscode.workspace.getConfiguration('cesaflow').update('apiKey', key, true);
            vscode.window.showInformationMessage('CesaFlow: API key saved');
            panel.refresh();
        }
    }), vscode.commands.registerCommand('cesaflow.refreshRuns', () => panel.refresh()), vscode.commands.registerCommand('cesaflow.cancelRun', async (runId) => {
        await api.cancelRun(runId);
        panel.refresh();
    }), vscode.commands.registerCommand('cesaflow.downloadRun', async (runId) => {
        vscode.window.showInformationMessage(`Download run ${runId} from the CesaFlow dashboard.`);
    }), vscode.commands.registerCommand('cesaflow.applyFiles', async (runId) => {
        await applyFilesToWorkspace(api, runId);
    }));
    // ── Phase 2: Inline edit (Ctrl+K), Chat (Ctrl+L), Tab completion ──────────
    ...
    statusBar.command = 'cesaflow.openPanel';
...
async function applyFilesToWorkspace(api, runId) {
    ...
    if (!workspaceFolder) {
        const action = await vscode.window.showErrorMessage('CesaFlow: No workspace folder open. Open a folder first.', 'Open Folder');
        if (action)
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'CesaFlow: Applying files...', cancellable: false }, async (progress) => {
        try {
            ...
            const action = await vscode.window.showInformationMessage(`CesaFlow: ${files.length} files applied`, 'Open Explorer');
            if (action)
