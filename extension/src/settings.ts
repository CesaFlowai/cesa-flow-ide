...
export function registerSettings(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand('orkestra.settings', () => showSettings(context))
  );
}

export function showSettings(context: vscode.ExtensionContext) {
  ...
  const currentUrl = cfg.get<string>('serverUrl', 'http://localhost:8001');
  const autoApply = cfg.get<boolean>('autoApplyFiles', false);
  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    switch (msg.type) {
      case 'save': {
        ...
        if (msg.apiKey !== undefined) await c.update('apiKey', msg.apiKey, true);
        if (msg.serverUrl !== undefined) await c.update('serverUrl', msg.serverUrl, true);
        if (msg.autoApply !== undefined) await c.update('autoApplyFiles', msg.autoApply, true);
